# BlueCore Technologies LLC Website

Static company website for **BlueCore Technologies LLC**.

## Overview

BlueCore Technologies LLC is a single-member U.S. LLC operating web properties in tools, gaming content, and affiliate publishing.

This repository contains a lightweight static website built with plain HTML and CSS. It can be hosted on GitHub Pages, Cloudflare Pages, Netlify, Vercel, or any standard static hosting service.

## Pages

- `index.html` - homepage
- `products.html` - launched websites
- `services.html` - operating areas and services
- `about.html` - company overview
- `contact.html` - contact information
- `privacy.html` - privacy policy
- `terms.html` - terms of use
- `affiliate-disclosure.html` - affiliate disclosure

## Company Information

- Company: BlueCore Technologies LLC
- Address: 30 N Gould St Ste R, Sheridan, WY 82801 US
- Email: support@bluecoretechnologiesllc.com
- Domain: bluecoretechnologiesllc.com

## Local Preview

Run a local static server from the project directory:

```bash
python3 -m http.server 4173 --bind 127.0.0.1
```

Then open:

```text
http://127.0.0.1:4173/
```

## Deployment Notes

For GitHub Pages, deploy from the repository root using the `main` branch.

For a custom domain, configure the DNS records for `bluecoretechnologiesllc.com` with the hosting provider and enable HTTPS.
